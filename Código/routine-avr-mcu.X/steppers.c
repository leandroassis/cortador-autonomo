...
return_code init_step_sequences(step_sequence *steps_5v_motor, step_sequence *steps_12v_motor1, step_sequence *steps_12v_motor2){
    ...
    for(int i = 0; i < 8; i++) steps_5v_motor = (step_sequence) malloc(sizeof(step_sequence));
    
    // Step Sequence for 12V stepper motors
    for(int i = 0; i < 4; i++) steps_12v_motor1 = (step_sequence) malloc(sizeof(step_sequence));
    for(int i = 0; i < 4; i++) steps_12v_motor2 = (step_sequence) malloc(sizeof(step_sequence));
    
    if(steps_5v_motor == NULL || steps_12v_motor1 = NULL || steps_12v_motor2 = NULL) return ERROR_ALLOCATING_MEMORY;

    for(int i = 0; i < 7; i++) steps_5v_motor[i]->next = &steps_5v_motor[i+1];
